...
try:
    ...
    Composer = None  # si falta, devolvemos un error claro al usarlo
import random
...
def _grupos_generar_doc_impl(idgrupo: int, formato: str):
    ...
    try:
        ...
        base_name = f"examen_{clave}_{ts}.docx"
       ...
       # ⚠️ NO toques numbering/secciones cuando usamos COM
        try:
            reparar_docx_inplace(destino_docx)  # OpenAndRepair + SaveAs
        except Exception:
            pass
        ...
        except DocxVacioError as e:
            return jsonify({
                "error": "Hay preguntas .docx sin contenido (documento en blanco).",
                "archivos_defectuosos": [{"path": p, "motivo": "contenido vacío"} for p in e.paths]
            }), 422
        ...
        result = {
            ...
            "ruta_rel": f"descargas/{base_name}",
        ...
        if formato == "pdf":
            ...
            pdf_path_tmp = generar_pdf(destino_docx)
            pdf_final = os.path.join(app.config['DESCARGAS_FOLDER'], pdf_name)
            try:
                shutil.move(pdf_path_tmp, pdf_final)
            except Exception:
                shutil.copy2(pdf_path_tmp, pdf_final)
                try: os.remove(pdf_path_tmp)
                except: pass
            result.update({
                "archivo_pdf": pdf_name,
                "ruta_rel_pdf": f"descargas/{pdf_name}"
            })
...
def _merge_with_python(paths, out_path):
    """
    Une DOCX con python-docx + docxcompose (sin Word/COM).
    SIN insertar salto de página manual.
    Luego normaliza las listas decimales de nivel 0 para
    que la numeración sea continua a través de todo el documento.
    """
    ...
    if not paths:
        raise ValueError("SELECCION_VACIA")

    ok, malos = [], []
    for p in paths:
        try:
            # 👉 usa la que ya tienes definida
            if _tiene_texto_o_contenido(p):
                ok.append(p)
            else:
                malos.append((p, "sin texto/table/imagen/ecuación detectable"))
        except Exception as e:
            malos.append((p, f"no pude leer document.xml: {e}"))

    if not ok:
        raise DocxVacioError([m[0] for m in malos])

    # Unir sin page-break extra
    maestro = DocxDocument(ok[0])
    ...
    # Unifica numeración decimal nivel 0 (corrida 1,2,3… sin tocar bullets/subniveles)
    try:
        _unificar_numeraciones_decimales(out_path)
    except Exception:
        pass
...
def _unir_docs_y_normalizar(archivos_docx: list[str], destino_docx: str):
    import os, shutil, tempfile

    rutas = []
    for p in sorted((archivos_docx or []), key=lambda s: os.path.basename(s).lower()):
        if not p: continue
        a = os.path.abspath(p)
        if os.path.exists(a): rutas.append(a)
    if not rutas:
        raise ValueError("SELECCION_VACIA")

    tmpdir = tempfile.mkdtemp(prefix="merge_q_")
    rutas_cortas = []
    for i, a in enumerate(rutas, start=1):
        dst = os.path.join(tmpdir, f"q{i:04}.docx")
        try:
            shutil.copy2(a, dst)
            rutas_cortas.append(dst)
        except Exception:
            pass

    # ⬇️ usar SIEMPRE la ruta Python (estable)
    out_path, malos_apertura, malos_merge = _merge_with_python(rutas_cortas, destino_docx)

    try: shutil.rmtree(tmpdir, ignore_errors=True)
    except: pass

    return out_path, malos_apertura, malos_merge
...
def _normalizar_numeracion_decimal(docx_path: str, target_numId: str = "1"):
    """
    Para todos los párrafos con numeración 'decimal' en ilvl=0, fuerza numId=target_numId, ilvl=0.
    Si numbering.xml no define ese numId decimal, lo crea.
    """
    ...
    # ---- numbering.xml: asegúrate de que target_numId sea decimal nivel 0
    num_map = {}  # (numId, ilvl) -> fmt
    if "word/numbering.xml" in files:
        num_root = ET.fromstring(files["word/numbering.xml"])
                # ... ya creaste num_root y asignaste files["word/numbering.xml"] ...
        _ensure_numbering_part(files)   # ← ¡clave!

        # mapa actual
        for num in num_root.findall(".//w:num", ns):
            numId = num.get(W + "numId")
            absId_el = num.find("./w:abstractNumId", ns)
            if not absId_el: 
                continue
            absId = absId_el.get(W + "val")
            absN = num_root.find(f".//w:abstractNum[@w:abstractNumId='{absId}']", ns)
            if absN is None:
                continue
            for lvl in absN.findall("./w:lvl", ns):
                ...
                if fmt is not None:
                    num_map[(numId, ilvl)] = fmt.get(W + "val")

        # ¿existe target_numId decimal en ilvl 0?
        if (target_numId, "0") not in num_map or num_map[(target_numId, "0")] != "decimal":
            # crea abstractNum y num mínimos para decimal
            import random
            # crea un abstractNumId libre
            used = {a.get(W+"abstractNumId") for a in num_root.findall(".//w:abstractNum", ns)}
            new_abs = "9999"
            while new_abs in used:
                new_abs = str(random.randint(1000, 9999))

            abs_el = ET.Element(W+"abstractNum", {W+"abstractNumId": new_abs})
            lvl0  = ET.SubElement(abs_el, W+"lvl", {W+"ilvl": "0"})
            ET.SubElement(lvl0, W+"numFmt", {W+"val": "decimal"})
            ET.SubElement(lvl0, W+"start", {W+"val": "1"})
            ET.SubElement(lvl0, W+"lvlText", {W+"val": "%1."})
            num_root.append(abs_el)

            # crea <w:num w:numId=target_numId>
            nums_usados = {n.get(W+"numId") for n in num_root.findall(".//w:num", ns)}
            # si ya existe target_numId, lo reciclamos; si no, lo creamos
            if target_numId not in nums_usados:
                n_el = ET.Element(W+"num", {W+"numId": target_numId})
                ET.SubElement(n_el, W+"abstractNumId", {W+"val": new_abs})
                num_root.append(n_el)

            files["word/numbering.xml"] = ET.tostring(num_root, encoding="utf-8", xml_declaration=True)

    # ---- document.xml: fuerza numId/ilvl
    if "word/document.xml" in files:
        ...
        for p in root.findall(".//w:p", ns):
            ...
            ilvl_val = ilvl.get(W+"val") if ilvl is not None else None
            numId_val = numId.get(W+"val") if numId is not None else None

            # ¿formato decimal?
            fmt = None
            if numId_val is not None and ilvl_val is not None and ("word/numbering.xml" in files):
                # usamos num_map leído arriba si estaba
                fmt = num_map.get((numId_val, ilvl_val))
            # si no lo sabemos, asumimos decimal y normalizamos igual (harmless)

            if fmt in (None, "decimal") and (ilvl is None or ilvl_val == "0"):
                if numId is None:
                    numId = ET.SubElement(numPr, W+"numId")
                numId.set(W+"val", target_numId)
...
def _unificar_numeraciones_decimales(docx_path: str) -> None:
    import zipfile, xml.etree.ElementTree as ET
    NS_W = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
    W = "{%s}" % NS_W
    ns = {"w": NS_W}
    ...
    if "word/numbering.xml" not in files or "word/document.xml" not in files:
        return

    # --- Lee numbering: mapea (numId, ilvl) -> fmt, y encuentra un 'anchor_numId' decimal en ilvl 0.
    num_map = {}
    anchor_numId = None
    num_root = ET.fromstring(files["word/numbering.xml"])
    for num in num_root.findall(".//w:num", ns):
        ...
        for lvl in absn.findall("./w:lvl", ns):
            ...
            fmt = nf.get(W+"val") if nf is not None else None
            if fmt:
                num_map[(numId, ilvl)] = fmt
                # ¿sirve de ancla? (decimal, nivel 0)
                if fmt in ("decimal", "decimalZero") and ilvl == "0" and anchor_numId is None:
                    anchor_numId = numId

    if not anchor_numId:
        # no hay listas decimales; nada que unificar
        return

    # --- Reescribe document.xml: solo párrafos con fmt decimal e ilvl=0 → numId=anchor_numId
    doc_root = ET.fromstring(files["word/document.xml"])
    changed = False
    for p in doc_root.findall(".//w:p", ns):
        numPr = p.find("./w:pPr/w:numPr", ns) or p.find(".//w:numPr", ns)
        if numPr is None:
            continue
        numId_el = numPr.find("./w:numId", ns)
        ilvl_el  = numPr.find("./w:ilvl", ns)

        # Si falta ilvl, trátalo como 0 (común en nivel 0)
        ilvl_val = (ilvl_el.get(W+"val") if ilvl_el is not None else "0")
        numId_val = (numId_el.get(W+"val") if numId_el is not None else None)

        fmt = num_map.get((numId_val or "", ilvl_val or "0"))
        if fmt in ("decimal", "decimalZero") and ilvl_val == "0":
            if numId_el is None:
                numId_el = ET.SubElement(numPr, W+"numId")
            if numId_el.get(W+"val") != anchor_numId:
                numId_el.set(W+"val", anchor_numId)
                changed = True
            # garantiza ilvl=0 explícito (opcional pero ayuda)
            if ilvl_el is None:
                ilvl_el = ET.SubElement(numPr, W+"ilvl")
                ilvl_el.set(W+"val", "0")
                changed = True
...
def _forzar_secuencia_preguntas(docx_path: str, target_numId: str = "1"):
    """
    Convierte encabezados de pregunta escritos como texto (p.ej. '1) ', '2. ', '(3) ')
    en una lista numerada decimal nivel 0 y une TODAS las listas decimales de nivel 0
    al mismo numId (target_numId). No toca bullets/letras/romanos.
    """
    ...
    # Detecta "1) ", "1. ", "(1) ", "1 " o pegado a texto.
    RX_Q = re.compile(r"^\s*\(?[1-9]\d{0,2}\)?[.)]?(?:\s+|(?=[^\s]))")
    ...
    # ---------- 1) LEER NUMBERING Y MAPEAR FORMATO REAL ----------
    num_fmt_map = {}  # (numId, ilvl) -> numFmt (decimal, bullet, upperLetter, etc.)
    if "word/numbering.xml" in files:
        num_root = ET.fromstring(files["word/numbering.xml"])
        for num in num_root.findall(".//w:num", ns):
            numId = num.get(W + "numId")
            ...
            for lvl in absn.findall("./w:lvl", ns):
                ...
                nf = lvl.find("./w:numFmt", ns)
                if nf is not None:
                    num_fmt_map[(numId, ilvl)] = nf.get(W + "val")
    else:
        num_root = ET.Element(W + "numbering")

    # Asegura que exista un numId decimal ilvl=0 para anclar (target_numId)
    have_decimal_anchor = any(
        (k[0] == target_numId and k[1] == "0" and v in ("decimal", "decimalZero"))
        for k, v in num_fmt_map.items()
    )
    if not have_decimal_anchor:
        used_abs = {a.get(W + "abstractNumId") for a in num_root.findall(".//w:abstractNum", ns)}
        new_abs = "9001"
        while new_abs in used_abs:
            new_abs = str(int(new_abs) + 1)
        ...
        files["word/numbering.xml"] = ET.tostring(num_root, encoding="utf-8", xml_declaration=True)
        _ensure_numbering_part(files)  # mantiene rels y content-types

        # refresca el mapa
        num_fmt_map[(target_numId, "0")] = "decimal"
    else:
        _ensure_numbering_part(files)

    # ---------- 2) EDITAR DOCUMENT.XML ----------
    root = ET.fromstring(files["word/document.xml"])

    def _get_para_text(p):
        return "".join((t.text or "") for t in p.findall(".//w:t", ns))

    def _strip_prefix_in_runs(p, match_len):
        """Quita 'match_len' caracteres del inicio del párrafo, repartidos en runs."""
        remaining = match_len
    ...
    for p in root.findall(".//w:p", ns):
        pPr = p.find("./w:pPr", ns) or ET.SubElement(p, W + "pPr")
        numPr = pPr.find("./w:numPr", ns)

        # --- Caso A: ya es lista. Solo unificar si es DECIMAL nivel 0 ---
        if numPr is not None:
            pPr = _ensure_pPr_at_start(p, W, ns)
            numPr, numId_el, ilvl_el = _ensure_numPr(pPr, W, ns)
            ...
            fmt = num_fmt_map.get((numId_val or "", ilvl_val or "0"))
            if ilvl_val == "0" and fmt in ("decimal", "decimalZero"):
                if numId_el is None:
                    numId_el = ET.SubElement(numPr, W + "numId")
                if ilvl_el is None:
                    ilvl_el = ET.SubElement(numPr, W + "ilvl")

                if numId_el.get(W + "val") != target_numId:
                    numId_el.set(W + "val", target_numId); changed = True
                if ilvl_el.get(W + "val") != "0":
                    ilvl_el.set(W + "val", "0"); changed = True
                continue  # no tocar bullets/letras/romanos

        # --- Caso B: párrafo con número "escrito" (texto) → convertir a lista decimal ---
        plain = _get_para_text(p)
        m = RX_Q.match(plain)
        if not m:
            continue

        # quita el prefijo textual "1) ", "2. ", "(3) ", etc.
        _strip_prefix_in_runs(p, m.end())

        # aplica numPr con el ancla decimal nivel 0
        pPr = _ensure_pPr_at_start(p, W, ns)
        numPr, numId, ilvl = _ensure_numPr(pPr, W, ns)


        if numId.get(W + "val") != target_numId:
            numId.set(W + "val", target_numId); changed = True
        if ilvl.get(W + "val") != "0":
            ilvl.set(W + "val", "0"); changed = True
...
def _normalizar_alternativas(docx_path: str):
    import re, zipfile, xml.etree.ElementTree as ET
    NS_W = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
    W = "{%s}" % NS_W
    ns = {"w": NS_W}
    ...
    # ✅ AQUI estaba el bug: siempre define num_root
    if "word/numbering.xml" in files and files["word/numbering.xml"]:
        num_root = ET.fromstring(files["word/numbering.xml"])
    else:
        num_root = ET.Element(W + "numbering")
        files["word/numbering.xml"] = ET.tostring(num_root, encoding="utf-8", xml_declaration=True)

    _ensure_numbering_part(files)  # mantiene rels/CT coherentes



def _ensure_pPr_at_start(p, W_ns, ns):
    """Devuelve <w:pPr> y se asegura de que sea el PRIMER hijo de <w:p>."""
    pPr = p.find("./w:pPr", ns)
    if pPr is None:
        pPr = ET.Element(W_ns + "pPr")
        # insertar al inicio
        p.insert(0, pPr)
    else:
        # si no es el primero, muévelo al inicio
        if list(p)[0] is not pPr:
            p.remove(pPr)
            p.insert(0, pPr)
    return pPr
...
def _ensure_numPr(pPr, W_ns, ns):
    """Devuelve <w:numPr> como primer hijo de <w:pPr> (antes de otras props)."""
    numPr = pPr.find("./w:numPr", ns)
    if numPr is None:
        numPr = ET.Element(W_ns + "numPr")
        pPr.insert(0, numPr)   # importante: al comienzo de pPr
    else:
        if list(pPr)[0] is not numPr:
            pPr.remove(numPr)
            pPr.insert(0, numPr)
    # hijos básicos
    numId = numPr.find("./w:numId", ns) or ET.SubElement(numPr, W_ns + "numId")
    ilvl  = numPr.find("./w:ilvl",  ns) or ET.SubElement(numPr, W_ns + "ilvl")
    return numPr, numId, ilvl
